# claude-loop 調査・設計レポート — Loop Engineering と LoopAgent

> 本レポートは claude-loop プロジェクトの **調査・設計フェーズ**の成果物である。実装は行わず、(1) Loop Engineering / LoopAgent の徹底調査、(2) claude-org-ja の資産棚卸しと再利用評価、(3) LoopAgent の設計（複数案比較→推奨1案）と段階ロードマップ、をまとめる。
>
> - 版: v1.0（2026-06-27）
> - 対象リポジトリ: `https://github.com/happy-ryo/claude-loop`
...
**本レポートの結論（推奨設計）**: claude-loop の LoopAgent は、
...
**claude-org-ja の資産再利用**: 調査の結果、claude-loop が必要とする要素の大半は claude-org-ja に既製の高品質実装が存在する。特に以下は再利用価値が高い:
...
claude-loop は、本格的な **Loop Engineering** を実現する **LoopAgent** の設計・実装プロジェクトである。本レポートはその起点として、
...
2023年4月前後に登場した第一世代（AutoGPT / BabyAGI / AgentGPT）は、いずれも「タスク生成 → 実行 → 再計画」を `while True` で回す素朴なループを核とした。**そのほとんどが実用で苦戦し、現代のループ設計原則は「その失敗の裏返し」として収束した**。これは claude-loop が踏むべきでない轍の宝庫であり、特に重視する。
...
以上を claude-loop の設計原則として10点に蒸留する。
...
| Role Contract（4役割の責務/境界） | `docs/contracts/role-contract.md`, 各 `CLAUDE.md` | **extract-pattern** | 「誰がループを回すか・どこで責務が切れるか」の骨格。claude-loop は coordinator / loop-agent / eval-agent の3段化が想定。 |
...
**SQLite `state.db`** が runs / org_sessions / events / worker_dirs / projects / workstreams の単一 SoT。markdown / JSON は snapshotter が DB から自動再生成する派生物で、drift_check が手書き編集を検出する。**claude-loop のループ状態（iteration, convergence history, termination 評価）永続化に直接再利用できる**最重要資産。
...
**最大の発見**: claude-loop が必要とする「終了条件を保存して再開」「状態の正本性」「フィードバック→改善」「観測→人間ゲート」は、**claude-org にすでに本番品質で存在する**。claude-loop は車輪の再発明をせず、これらを **runtime 非依存な抽象（state DB / transport / feedback / gate）として段階的に抽出・再利用**することが最短経路である。
...
§2 の調査と §3 の資産から、claude-loop の LoopAgent が満たすべき要件:
...
- **短所**: **重い**。pane/tmux/renga/broker 等の runtime 依存が大きく、claude-loop 単体プロジェクトには過剰。人間ゲートが組織運用前提で過多になりがち。PoC に向かない。
...
- **transport の runtime 依存**: broker sidecar 等は runtime 所属で直接再利用不可[^framework-common]。→ パターン（push一次/pull fallback, at-most-once, role 別 cadence）のみ抽出し、claude-loop 側の配送実体は別実装。
...
*本レポートは claude-loop の調査・設計フェーズ成果物（v1.0, 2026-06-27）。調査は ultracode workflow による fan-out（19 エージェント: org 資産棚卸し7サブシステム + web 調査6サブ問 + 独立反証検証6件）で実施し、主張は上記出典で裏付け、主要主張は独立エージェントで反証検証した。*
