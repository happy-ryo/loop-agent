...
- **multi-item 公平 scheduling `WorkListGather`**（Issue #56）: N 件を 1 本のループで
  ...
  に委譲する `WorkListGather.from_triage(...)` を同梱。#37 Self-translation PoC で手書き
  した round-robin pattern の正規化（`loop_agent.discovery.work_list`）。
  - **内部変更**: `loop_agent.discovery` を単一モジュールから package 化（`triage` /
    `work_list` サブモジュール）。公開 import（`from loop_agent import ...` /
