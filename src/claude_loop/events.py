...
イベントは与えられた run に対して決定的になる（:mod:`claude_loop.progress`
と同じ方針）。
"""
...
def _jsonable(value: Any) -> Any:
    ...
    :func:`claude_loop.progress._to_jsonable` と同じ方針で、保存される形を予測可能に
...
class EventSink(Protocol):
    ...
    :class:`~claude_loop.observe.LoopObserver` を参照）ので、実装は理想的には
    例外を投げないことが望ましい。
    """
...
# 観測層（:mod:`claude_loop.observe`）でもこの型を共有する。
...
def read_events(path: "str | os.PathLike[str]") -> list[dict[str, Any]]:
    ...
    隠さない。ファイルが無ければ空 list を返す（:func:`claude_loop.progress.read_progress`
