...
class LoopStore:
    ...
    def load_or_init(self, run_id: str) -> LoopState:
        ...
        である (:func:`claude_loop.progress._to_jsonable` が tuple->list / 非 JSON
        ネイティブ型 ->repr 文字列 / dict キー ->str に coerce する)。よって
        observation を直接 *キー* にする state ベース条件 (特に
        :class:`~claude_loop.conditions.NoProgress` の既定 key) は、JSON 安定な
        observation 型で使うか、JSON 安定な signature へ射影する ``key`` を渡すこと
        (詳細は :func:`claude_loop.loop.run_loop` の ``initial_state`` 参照)。
        """
        if not run_id:
    ...
    def complete_execution(self, run_id: str, gate_key: str, owner: str) -> bool:
        ...
        防ぐ (driver は :attr:`claude_loop.loop.GateReview.on_complete` でこの順序を保証する)。
        """
        if not owner:
