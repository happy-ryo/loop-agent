...
- **不可逆 action は resume をまたいで exactly-once + 順序整合 (Issue #21)**。approve/edit で
  実行する不可逆 action は :meth:`~claude_loop.store.LoopStore.acquire_lease` で single-winner
  に **in-progress リース** を取得し (``resolved -> executing``)、``act`` 完了後に
  :meth:`~claude_loop.store.LoopStore.complete_execution` で ``executed`` を確定する。
...
(:meth:`~claude_loop.store.LoopStore.acquire_lease` の ``resolved -> executing`` single-winner
遷移) で 1 者だけが実行権を得る。これにより:

- **exactly-once 実行**: ``resolved -> executing`` に成功するのは 1 プロセスだけ。勝者が
  ``act`` を実行し :meth:`~claude_loop.store.LoopStore.complete_execution` で ``executed`` を
...
- **勝者クラッシュ復旧**: 勝者が ``act`` 途中でクラッシュしリースが失効
  ...
  :attr:`claude_loop.loop.GateReview.on_complete` を on_step の後に呼ぶ)、勝者クラッシュでも
  step が欠落しない。
...
class HumanGate:
    ...
    Args:
        ...
        owner: 任意。in-progress リースの保持者トークン (Issue #21)。同一 run_id を resume
            ...
            **resume 試行毎 (プロセス再起動を含む) に一意**であることが要件: 別試行と owner が
            ...
            (:meth:`~claude_loop.store.LoopStore.acquire_lease` の再入分岐)。安定 owner を跨いで
        ...
        lease_ttl: in-progress リースの TTL (秒)。不可逆 action 1 回の実行所要より十分長く
            ...
            なりうる)。既定 :data:`~claude_loop.store.DEFAULT_LEASE_TTL`。
    ...
    def review(self, context: Any, state: LoopState) -> GateReview:
        ...
        - ``resolved`` / ``executing``: action 一致を確認のうえ適用 (approve/edit は
          :meth:`~claude_loop.store.LoopStore.acquire_lease` で in-progress リースを取得し、
    ...
    def _make_on_complete(self, gate_key: str) -> Callable[[], None]:
        ...
        driver は step を永続化した *後* にこれを呼ぶ (:func:`claude_loop.loop.run_loop`)。
        :meth:`~claude_loop.store.LoopStore.complete_execution` が ``executing -> executed``
    ...
    def _apply_resolved(
        ...
        approve/edit は in-progress リース (:meth:`~claude_loop.store.LoopStore.acquire_lease`)
