...
**既知の制限 (resume の集計復元は #14)**: resume は loop レベルの **累積集計**
(``state.tokens_used`` / ``elapsed`` / ``iteration``) を復元しない (毎 run iteration 0 から
再生し、既実行ゲートの skip は ``tokens=0`` の非永続 step で読み飛ばす)。このため
**run を跨いで累積する上限** (:class:`~claude_loop.conditions.TokenBudget` /
:class:`~claude_loop.conditions.Timeout`) を gate の pause/resume 越しに信頼してはならない
(再開後は前 run までの消費がリセットされて見える)。各 step の正本は ``step`` 行に残るので
監査・コスト集計はそこから行う。累積集計を引き継いだ完全な resume は #14 (loop-state 復元)
の領分。
