...
**既知の制限 (resume の状態復元は #14)**: resume は loop レベルの状態を復元しない
(毎 run iteration 0 から再生し、既実行ゲートの skip は ``tokens=0`` の非永続 step で
読み飛ばす)。具体的には:

- **累積集計**: ``state.tokens_used`` / ``elapsed`` / ``iteration`` は復元されない。よって
  **run を跨いで累積する上限** (:class:`~claude_loop.conditions.TokenBudget` /
  :class:`~claude_loop.conditions.Timeout`) を gate の pause/resume 越しに信頼しては
  ならない (再開後は前 run までの消費がリセットされて見える)。
- **観測履歴**: 既実行ゲートの skip は合成 placeholder を ``state.history`` に積むため、
  ``gather`` が ``state.history[-1]`` 等 **過去の observation 内容に依存して提案を変える**
  ループでは、resume 後の提案列が初回と乖離しうる (後続ゲートの action 不一致や別の
  reversible action 実行の原因)。本 MVP は「提案列は resume 間で決定的かつ履歴内容に
  依存しない」ことを前提とする。

各 step の正本は ``step`` 行に残るので監査・コスト集計はそこから行う。累積集計・観測履歴を
引き継いだ完全な resume (history-dependent な再開) は #14 (loop-state 復元) の領分。
