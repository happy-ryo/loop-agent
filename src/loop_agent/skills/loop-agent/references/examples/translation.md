...
| `verify` | **done only when all 4 mechanical checks pass** (below) | asking an LLM-as-judge "did you translate it?" converges on "pretending it's done". Measure success against ground truth. |
...
## verify is the crux - ground truth in 4 stages ordered by ascending cost

A file becomes **done** only when all 4 checks pass. Order them from the cheapest gate so broken edits get rejected early:
...
4. **`tests_pass`** - that module's own `pytest` passes (re-importing in a subprocess detects behavioral breakage).
