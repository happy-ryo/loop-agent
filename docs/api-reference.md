...
LoopAgent が公開する API の索引ページ。`1.0.0` の安定スコープ、インストール手順、エクスポートされる全要素の一覧表、テストスイートのカバレッジをまとめる。

## 1.0.0 Stable のスコープ

report.md §4.4 から始まったループコアを、組み込み用の安定ランタイムとして拡張した実装。**単一エージェント・単一プロセス**で `gather → act → review? → verify → repeat` を回し、**合成可能なハード上限**（`MaxIterations` / `TokenBudget` / `Timeout`）を OR 評価する。上限到達は**例外ではなく理由付きの制御出力**（`LoopResult`）で返る。互換性の境界は [stability.md](./stability.md) が正本である。

スコープ（欲張らない = *simpler loops win*）:

- ✅ ループドライバ + 機械的な合成 stop 条件（発火した条件と理由を保持）
- ✅ `act` / `review` / `verify` は**注入可能なフック**（in-memory 関数 / subprocess / Claude Code / Codex / 自作 adapter を同じシームに載せる）
- ✅ ground-truth verifier helper: `CommandVerifier` / `PytestVerifier` / `RegexVerifier` は、既存の機械的 oracle を `verify` seam に載せる薄い補助。成功判定の policy は呼び出し側に残す
...
- ✅ **async/await 対応**: 非同期エントリポイント `async_run_loop`（`await async_run_loop(…)`）。同期 API `run_loop` は完全維持（内部は `asyncio.run` ラッパ）。`gather`/`act`/`review`/`verify`/`conditions`/`gate`/`on_step` の各シームは **同期 callable のまま受けつつ非同期（acallable）も受ける**（混在可・同期フックは追加コストなし）。`asyncio.gather` で複数ループを並行実行できる（`tests/test_async_loop.py` / Issue #40）
- ✅ **限定人間ゲート**: 不可逆操作のみ approve/edit/reject/respond で interrupt（state 永続化で pause/resume・不可逆は exactly-once。Issue #15）
- ✅ **optional post-act review**: `ReviewOutcome` を返す `review=` フックで、LLM-backed `act` の成果物を `verify` 前に評価する。blocking review は failed step として JSON feedback を残し、その iteration の `verify` をスキップする（Issue #128）
...
- ✅ **運用 read-only helpers**: `summary` / 静的 HTML `dashboard` / post-hoc `spikes` / live `SpikeDetector` / circuit breaker `StopCondition` helpers / opt-in throttling primitives（[operations-roadmap.md](./operations-roadmap.md)）。Grafana 等の外部基盤と事業固有の自動制御 policy は呼び出し側が持つ

## 安定 API 境界

`1.0.0` では、`loop_agent.__all__` にあるシンボルを公開 API として扱う。そのうち、日常利用で安定契約の中心になる core API と、高度だが互換性を維持する advanced API の分類は [stability.md](./stability.md) にまとめる。公開 API の削除・改名・非互換なシグネチャ変更は major release を要する。


`loop_agent` の top-level は coding agent が harness に必要な部品を発見しやすいよう広めに維持する。ただし平坦な一覧ではなく、`CORE_API` / `HARNESS_API` / `ADVANCED_API` / `OPERATIONS_API` / `PUBLIC_API_GROUPS` で分類する。人間向けの最短 import は [first-harness-api.md](./first-harness-api.md)、coding agent 向けの用途別 import map は [ai-api-map.md](./ai-api-map.md) を正とする。
...
- `gather` を省略すると `LoopState` がそのまま `act` の context になる。`review(outcome)` は任意の post-act hook で、blocking review は `verify` をスキップして feedback を次 iteration へ残す。`on_step(record, state)` は各反復完了後に呼ばれる最小の観測フック。
...
- [seams.md](./seams.md) — シーム（gather/act/review/verify/conditions/gate）の詳細仕様と型
...
- [verifiers.md](./verifiers.md) — ground-truth verifier helper
- [ai-api-map.md](./ai-api-map.md) — coding agent 向け capability map
- [api-surface.md](./api-surface.md) — public API 追加基準
