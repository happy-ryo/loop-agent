...
report.md §4.4 から始まったループコアを、組み込み用の安定ランタイムとして拡張した実装。**単一エージェント・単一プロセス**で `gather → act → verify → repeat` を回し、**合成可能なハード上限**（`MaxIterations` / `TokenBudget` / `Timeout`）を OR 評価する。上限到達は**例外ではなく理由付きの制御出力**（`LoopResult`）で返る。互換性の境界は [stability.md](./stability.md) が正本である。
