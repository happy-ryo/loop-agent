...
`loop_agent` の top-level は coding agent が harness に必要な部品を発見しやすいよう広めに維持する。ただし平坦な一覧ではなく、`CORE_API` / `HARNESS_API` / `ADVANCED_API` / `OPERATIONS_API` / `PUBLIC_API_GROUPS` で分類する。最短 import は [first-harness-api.md](./first-harness-api.md)、coding agent が用途から API を選ぶときは [ai-api-map.md](./ai-api-map.md) を使う。
