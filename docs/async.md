...
`on_timeout="graceful"` (the default) gives up, records a synthetic step, and proceeds to the next iteration. `"kill"` raises `SeamTimeout`. This is separate from the whole-run `Timeout` stop condition. See [recipes/timeout-and-kill.md](./recipes/timeout-and-kill.md) for details.

## Related

- [../README.md](../README.md) - LoopAgent overview and navigation
- [seams.md](./seams.md) - Types and contracts for gather / act / review / verify / conditions / gate / on_step seams
- [recipes/timeout-and-kill.md](./recipes/timeout-and-kill.md) - How to use per-call timeout (graceful / kill)
