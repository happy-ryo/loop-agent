...
report.md R3（無限ループ防止）の要請に対応する。stop 条件を 1 つも渡さないと `ConfigError` で拒否される（必ず発火する条件が無い構成を起動時に弾く）。条件 API の完全な一覧と合成セマンティクスは [api-reference.md](./api-reference.md) と [seams.md](./seams.md) を参照。
...
- [README](../README.md) — 全体像・positioning・シーム概観
- [seams.md](./seams.md) — シーム詳細仕様と型（conditions / gate / act の境界）
- [api-reference.md](./api-reference.md) — stop 条件と HumanGate の完全な API
