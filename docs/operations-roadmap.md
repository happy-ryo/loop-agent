...
最初に作るべき dashboard は、イベントをそのまま可視化する薄いものにする。
...
- JSONL event sink を読む CLI summary
...
検出結果は `event` として記録し、初期段階では run を止めない。止めるかどうかはアプリ側 policy が決める。
...
`NoProgress` で表現できるものはまず `NoProgress(key=...)` を使う。表現できない場合だけ専用 `StopCondition` を追加する。

## Tracking

- Dashboard: Issue #103
- Spike detection / throttling: Issue #103 の子タスク候補
- Circuit breaker recipes: Issue #103 の子タスク候補
