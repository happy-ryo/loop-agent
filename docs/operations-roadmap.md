...
このページは、その上に載せる運用 follow-up をまとめる。ここで扱うものは loop core の必須条件ではなく、長時間運用や複数 loop 運用で必要になる policy / UI / 自動制御である。

## Dashboard

最初に作るべき dashboard は、イベントをそのまま可視化する薄いものにする。0.1.0 の次の段階として、まず `loop-agent summary` が `state.db` の read-only run 一覧を提供する。
...
検出結果は `loop_spike` event として記録し、初期段階では run を止めない。止めるかどうかはアプリ側 policy が決める。`SpikeDetector` は `on_step` observer として opt-in で使える。
...
`NoProgress` で表現できるものはまず `NoProgress(key=...)` を使う。表現できない場合だけ専用 `StopCondition` を追加する。具体例は [recipes/circuit-breakers.md](./recipes/circuit-breakers.md)。
