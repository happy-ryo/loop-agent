...
- **async の cancel は協調的**。`asyncio.wait_for` は seam の次の await 点で `asyncio.CancelledError` を投げて cancel します。`CancelledError` を **握り潰して値を返す** seam は timeout を黙って無効化し（`SeamTimeout` が出ない）、握り潰した上で完了しない await をすると **ループがハング**します。timeout を効かせる seam で `CancelledError` を catch-and-ignore しないこと。`async def` seam 内の**同期ブロッキング区間**も、次に await するまでは中断できません。
