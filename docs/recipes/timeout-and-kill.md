...
- **per-call budget は区間ごと**。「同期ブロッキング → awaitable を返す」という変則 seam では、同期区間（`SIGALRM`）と await 区間（`wait_for`）がそれぞれ別に締切で縛られ、合計で最大 ~2 倍になりえます。単一の締切にしたいなら素直に `async def` にしてください。
