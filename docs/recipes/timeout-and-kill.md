...
| シーム | 中断機構 | 移植性 |
|---|---|---|
| **async**（`async def` / awaitable を返す） | `asyncio.wait_for` で await 点で実 cancel | **全プラットフォーム**（イベントループがあれば。= `async_run_loop`） |
...
- **`kill`** — 同期シームに対しては、呼び出しに入る **前**に `UnsupportedTimeoutKill` を送出します（縛れない hard kill を黙ってハングさせるより、明示エラーで失敗させる方針）。async シームなら `wait_for` で止まるので問題ありません。

その他の注意:

- **async の cancel は協調的**。timeout 時は seam の task を cancel し（次の await 点で `asyncio.CancelledError`）、seam の反応に関わらず timeout を報告します（締切時点で task が pending かで判定するので、`CancelledError` を握り潰して値を返す seam でも kill は効きます。seam 自身が投げた `asyncio.TimeoutError` は別物として伝播します）。ただし `CancelledError` を握り潰した上で**完了しない await** をすると cancel cleanup で**ハング**しうるので、timeout を効かせる seam で `CancelledError` を catch-and-ignore しないこと。`async def` seam 内の**同期ブロッキング区間**も、次に await するまでは中断できません。
- **per-call budget は単一**。締切は 1 回の呼び出し全体に効きます。「同期ブロッキング → awaitable を返す」変則 seam でも、同期区間で消費した時間は await 区間（`wait_for`）の予算から差し引かれ（同期区間で締切を使い切っていれば即 timeout）、合計は単一の締切で縛られます（~2 倍にはなりません）。
...
- **締切は実 wall-clock 基準**。`SIGALRM` / `wait_for` はループの注入クロック `time_fn` を見ません（`time_fn` は stop 条件用クロックと、非 `SIGALRM` の post-hoc 計測にのみ作用）。
