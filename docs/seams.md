...
> **verify は ground truth で書く（推奨）**: 何でも差せるのがシームの本質だが、成功判定を LLM-as-judge に委ねるとループは「成功したフリ」に収束しやすい（report.md R1）。pytest の exit-code / AST / 文字列スキャンなど機械的に判定できるものを使う。具体例は [recipes/](./recipes/)。
