...
ループの一生を **構造化イベント** として外に出す観測層。`run_observed_loop` にループを通すと、`loop_begin` → `loop_step` × N → `loop_end` のイベントが **sink** へ流れる。各イベントは反復番号・累積トークン・elapsed・**終了理由**を運び、ループが「なぜ・どう終わったか」を事後解析できる。同じ run は OTel が入っていれば 1 本の **GenAI span**（`gen_ai.*` + 反復番号 + 終了理由）にもなる。
...
- **OTel は optional 依存**: 未導入環境でも `ReflexionSpan` が **no-op に degrade** し、JSONL /
  event sink はそのまま機能する（内側 `LoopSpan` と同方針）。SDK を入れて span を実検査
...
**スコープ境界**: 本ページは **観測の emit 層**（events + OTel GenAI span への接続）を扱う。
**dashboard 化**（Grafana 等への可視化パイプライン）と **3x スパイク自動スロットル**（観測値を
使った自動制御＝外側ループへのフィードバック）は [operations roadmap](./operations-roadmap.md) で追跡する。
...
- [operations-roadmap.md](./operations-roadmap.md) — summary / dashboard / spike detection / throttling / circuit breaker の運用 helpers
